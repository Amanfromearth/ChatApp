...
const ChatListItem = ({
  ...
  return (
    <li
      className={`hover:bg-[#e9e9e9] transform-all ease-in-out duration-300 px-5 w-full ${
        currentSession?.id === session.id ? "bg-[#e9e9e9]" : ""
      }`}
    >
      <div
      ...
      >
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col h-full text-left justify-center items-center font-medium text-base">
            <p className="w-full text-left text-lg font-medium">
              {session.name}
            </p>
...
const Sidebar = ({
  ...
  return (
    <aside
      className={`transition-all ease-in-out  w-screen lg:w-fit h-full lg:flex gap-2 flex-col`}
    >
      ...
      <div className="h-full flex flex-col overflow-x-hidden  overflow-y-scroll rounded-2xl bg-front">
        <p className="w-full text-left text-sm text-zinc-400 m-5 font-medium">
          All
        </p>
